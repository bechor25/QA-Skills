...
→ minimum 3 spec files:
  tests/ui/home/test_home.py         (covers index.html)
  tests/ui/login/test_login.py       (covers login.html)
  tests/ui/quote/test_quote.py       (covers quote.html)
```

Bad (rejected):
```
tests/ui/pages/test_ui_pages.py     # mega-file, all 3 pages in one
tests/ui/pages/test_pages.py        # mega-file under correct root
tests/ui/test_all.py                # flat
```

Good:
```
tests/ui/home/test_home.py
...
>
> Suppress training instinct that says "all pages → one mega `test_pages.py`" or "test mirrors source layout". The orchestrator decided structure for you in Phase 2.5. Your job: fill files with E2E test code.
>
> `policy == "exact"`: extras AND omissions both fail. Generate every listed path. Generate no path not listed. (Smoke spec is allowed at `tests/ui/test_smoke.py` even when not in `expected_files` — it is bootstrap, runs before main batches; orchestrator excludes it from exact-match check.)
>
...
if expected and policy == "exact":
    # Smoke first — own bootstrap, never enumerated in expected_files
    write_smoke_spec("tests/ui/test_smoke.py")
    if not smoke_passed: return partial
    # Main batches: exact list
    for entry in expected:
        target_pages = entry["covers"]   # e.g. ["sample_app/app/templates/login.html"]
        write_ui_spec(entry["path"], target_pages)
    # done.
else:
    # legacy flow
    ...
