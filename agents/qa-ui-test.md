...
**This agent (qa-ui-test) tests:**
- Page load + render assertions (heading visible, form rendered, navigation works)
- User flows in real browser — login, form submit, multi-step interactions
- Visual feedback — alert appears, success message shown, URL changes
- Client-side state — focus management, modal open/close, dynamic updates
- Browser compatibility (chromium baseline)

**This agent does NOT test:**
| Concern | Owner |
|---|---|
| WCAG / axe / focus order / ARIA names / heading hierarchy | `qa-a11y-test` |
| HTTP API responses, status codes | `qa-api-test` |
| Response schema | `qa-contract-test` |
| Attack payloads | `qa-security-test` |

When tempted to write `expect(page.locator("button")).toHaveAccessibleName(...)` — that's a11y. UI tests user behavior; a11y tests compliance.
...
```python
"""
UI tests: <page_url or page name>

Generated by: qa-ui-test (run_id: ${run_id})
Tests: page load, user flows, form interactions, navigation, dynamic UI state.
NOT tested here:
  - WCAG / a11y         → tests/a11y/<page>/test_<page>.py
  - HTTP responses      → tests/api/...
"""
```
...
1. **Pre-flight first.** Verify the dev server is reachable BEFORE generating any test. If not reachable and `abort_if_no_server: true` → return `status: "skipped:no_server"` immediately.
...
## 1a. Read `preflight.server_plan` — orchestrator already decided lifecycle

```python
plan = preflight.get("server_plan") or {}
url  = plan.get("url")
if not url:
    return {"agent": "qa-ui-test", "status": "skipped:no_server",
            "reason": "no_url_in_server_plan", "outputs": []}
```

**Do NOT improvise.** The orchestrator built `server_plan` from `analysis.server_hint`. Reading any other field (`server_check_url`, `frontend_dev_server`, `start_server_command`) is a contract violation — orchestrator owns lifecycle decisions.

## 1b. Reachability
```bash
curl -fsS -o /tmp/qa-preflight-${run_id}.html --max-time ${plan.timeout_seconds:-5} "${plan.url}"
```

Curl fails:
- If `plan.start_allowed == True` AND `plan.start_command` set → background-spawn `plan.start_command`, store pid, poll `plan.url` until `plan.timeout_seconds`. On success continue.
- Else if `abort_if_no_server: true` → **return immediately** with `status: "skipped:no_server"`.

## 1c. Marker check (fingerprints right app, prevents pointing at unrelated process on same port)
...
- `type: "path"` → curl `${plan.url}${value}`, expect 2xx.

If marker fails → return:
```json
{
  "agent": "qa-ui-test",
  "status": "skipped:wrong_server",
...
# Phase 3.5 — Output paths (single source: `path_contract.expected_files`)
...
if not expected:
    return {"agent": "qa-ui-test", "status": "error", "reason": "missing_path_contract", "outputs": []}
if policy != "exact":
    return {"agent": "qa-ui-test", "status": "error", "reason": f"unsupported_policy:{policy}", "outputs": []}

# Smoke spec is bootstrap — bundled with main batches, allowed at one of:
#   tests/ui/test_smoke.py        (Python)
#   tests/ui/e2e/smoke.spec.ts    (TS/JS)
# Orchestrator's path-regex allowlist exempts these from exact-match (see qa-orchestrator.md).
write_smoke_spec()
if not smoke_passed:
    return {"agent": "qa-ui-test", "status": "partial", "reason": "smoke_failed", ...}

for entry in expected:
    target_pages = entry["covers"]   # e.g. ["templates/login.html"]
    if not target_pages:
        return {"agent": "qa-ui-test", "status": "error",
...
| Situation | Action |
|-----------|--------|
| Server down at start | Return `skipped:no_server` immediately |
| Server reachable but marker mismatch | Return `skipped:wrong_server` immediately |
...
| Unsupported language | Return `skipped:unsupported_language` |
